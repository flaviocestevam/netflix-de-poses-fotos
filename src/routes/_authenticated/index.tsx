  ...
  return (
    <div className="space-y-10">
      ...
      <section className="grid grid-cols-2 gap-3 md:grid-cols-4">
        {[
          { to: "/sos", label: "SOS Foto", icon: Sparkles, sub: "Resgate rápido" },
          { to: "/modo-30s", label: "Modo 30s", icon: Zap, sub: "Poses fáceis" },
          { to: "/cenarios", label: "Cenários", icon: Map, sub: "Onde estão" },
          { to: "/minha-viagem", label: "Minha Viagem", icon: Plane, sub: "Roteiros seus" },
        ].map((t) => {
          const I = t.icon;
          return (
            <Link key={t.to} to={t.to} className="group flex items-center gap-3 rounded-2xl border border-border/60 bg-card p-4 transition hover:border-primary/40">
      ...
      <Shelf title="SOS Foto Agora" subtitle="Quando ele está sem paciência" link="/sos" loading={sosQ.isLoading}>
        {sosQ.data?.map((p) => <PoseTile key={p.id} pose={p} />)}
      </Shelf>
      <Shelf title="Poses rápidas de 30 segundos" subtitle="Cenários do dia a dia" link="/modo-30s" loading={fastQ.isLoading}>
        {fastQ.data?.map((p) => <PoseTile key={p.id} pose={p} />)}
      </Shelf>
      <Shelf title="Onde vocês estão agora?" subtitle="Escolha o cenário" link="/cenarios" loading={categoriesQ.isLoading}>
        {categoriesQ.data?.filter((c) => c.category_type === "cenario").map((c) => <CategoryTile key={c.id} cat={c} />)}
      </Shelf>
      <Shelf title="Categorias mais usadas" subtitle="As favoritas das clientes" link="/cenarios" loading={categoriesQ.isLoading}>
        {categoriesQ.data?.filter((c) => c.category_type === "emocional").map((c) => <CategoryTile key={c.id} cat={c} />)}
      </Shelf>
      <Shelf title="Roteiros de viagem" subtitle="Sequências prontas de poses" link="/roteiros" loading={scriptsQ.isLoading}>
        {scriptsQ.data?.map((s) => (
          <Link key={s.id} to="/roteiros" className="group relative block w-[260px] shrink-0 overflow-hidden rounded-2xl border border-border/60 bg-card">
      ...
      <Shelf title="Minha Viagem" subtitle="Suas pastas de viagem" link="/minha-viagem" loading={tripsQ.isLoading}>
        {tripsQ.data?.map((t) => (
          <Link key={t.id} to="/minha-viagem" className="group relative block w-[260px] shrink-0 overflow-hidden rounded-2xl border border-border/60">
            <div className="relative aspect-[4/5]">
      ...
      <section className="rounded-3xl border border-border/60 bg-card/60 p-6 md:p-10">
        <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
          ...
          <Link to="/downloads" className="inline-flex items-center justify-center gap-2 rounded-full border border-border bg-secondary px-5 py-3 text-sm font-medium hover:bg-secondary/70">
            <Download className="h-4 w-4" /> Ver downloads
          </Link>
        </div>
      </section>
    </div>
...
function Shelf({ title, subtitle, link, loading, children }: { title: string; subtitle?: string; link?: any; loading?: boolean; children: React.ReactNode }) {
  return (
    <section>
      <div className="mb-3 flex items-end justify-between gap-3">
        ...
        {link && <Link to={link} className="shrink-0 text-xs font-medium text-primary hover:underline">Ver todas</Link>}
      </div>
...
function PoseTile({ pose }: { pose: any }) {
  return (
    <Link to="/" className="group relative block w-[180px] shrink-0 overflow-hidden rounded-2xl border border-border/60 bg-card md:w-[220px]">
...
function CategoryTile({ cat }: { cat: any }) {
  return (
    <Link to="/cenarios" className="group relative block w-[220px] shrink-0 overflow-hidden rounded-2xl border border-border/60 md:w-[260px]">
      <div className="relative aspect-[4/5]">
